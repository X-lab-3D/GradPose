...
# TODO: --chain optional, if not used: longest chain from template
# TOOD: Everything is optional, add (Optional) to everything and maybe allow using only one argument (no arg parse)
#       One argument = input folder
...
class Rotator():
    ...
    """
    def __init__(self, xyz, loss_mask, center=None, quaternions=None, use_numpy=False):
        """Initializes the Rotator object.

        Args:
            xyz (torch.Tensor): Coordinates for each atom for each PDB.
            loss_mask (torch.Tensor): A mask to zero out missing atoms when optimizing.
        ...
        self.center = self.get_center(xyz, loss_mask) \
            if center is None else center.to(DEVICE)
        ...
        if use_numpy:
            ...
            self.quaternions = torch.rand(self.nmb_samples, 4).to(DEVICE) \
                if quaternions is None else quaternions.to(DEVICE)
    ...
    def get_center(self, xyz, loss_mask):
        """Get the centers of each PDB. Ignoring missing residues.

        Args:
            xyz (torch.Tensor): PDB coordinates.
            loss_mask (torch.Tensor): Loss mask for each PDB.

        Returns:
            torch.Tensor: Tensor with the center of each PDB.
        """
        return (xyz*loss_mask).sum(1).div(loss_mask.sum(1)).reshape(-1,1,3)
    ...
    def get_matrix(self):
        ...
        if self.use_numpy:
            ...
            matrix = torch.zeros(q.shape[0], 3, 3).to(DEVICE)
        # Fill the matrix
    ...
    def _loss(self):
        ...
        loss =  F.mse_loss(new_xyz, self.xyz[0], reduction='none') * self.loss_mask
        loss = loss.mean(2)
        loss = loss.mean(1)
...
class PDBdataset():
    ...
    """
    def __init__(self, pdbs, template_pdb, residues, chain, cores=1, output='result', verbosity=1):
        ...
        if not residues or not chain:
            ...
            with open(template_pdb, 'r', encoding='utf-8') as template_file:
                ...
                if not chain:
                    ...
                    self.chain = sorted([(count, chain_id) for chain_id, count in chain_dict.items()], reverse=True)[0][1]
                    if self.verbosity > 0:
                        print(f"{self.chain}")
                ...
                if not residues:
                    ...
                    residues = list(set([int(line[22:26]) for line in template_file_data if line.startswith('ATOM ') and line[21] == self.chain]))
                    if self.verbosity > 0:
    ...
    def load_ca_atom_positions(self):
        ...
        with mp.Pool(self.cores) as pool:
            ca_xyz = torch.tensor(
                np.concatenate(list(tqdm(pool.imap(self._extract_pdb_ca, self.pdbs, max(2, round(len(self.pdbs) / (self.cores * 16)))),
                                         total=len(self.pdbs), desc="Extracting backbones",
                                         disable=self.verbosity!=1)))).float().to(DEVICE)
        loss_mask = (ca_xyz.abs().sum(2) != 0).unsqueeze(2).float().to(DEVICE)
        return ca_xyz, loss_mask
    ...
    def optimize(self):
        ...
        finetune_steps = 25
        step = 0
        for step in trange(0, shake_steps+finetune_steps, desc="Aligning backbones", disable=self.verbosity != 1):
            # Take a step
            loss = self.rotator.step()
            if step <= shake_steps:
                self.rotator.normalize_q()
            if step == shake_steps:
                self.rotator.optimizer = torch.optim.SGD([self.rotator.quaternions], lr=0.00001) # TODO deze 0.01 is ergens van afhankelijk
            if self.verbosity > 1:
...
def rmsd_test_pool(variables):
    unaligned_pdb, template, output, rmsd_dict, chain = variables
    pdb_basename = os.path.basename(unaligned_pdb)
    aligned_pdb = os.path.join(output, pdb_basename)


    # Align using pdb2sql
    aligned_sql = pdb2sql.superpose(unaligned_pdb, template, export=False)
    template_sql = pdb2sql.pdb2sql(template)

    a_res = set(aligned_sql.get("resSeq", chainID=[chain]))
    t_res = set(template_sql.get("resSeq", chainID=[chain]))
    intersect_res = a_res.intersection(t_res)
    try:
        pdb2sql_rmsd = pdb2sql.StructureSimilarity.get_rmsd(
            np.array(aligned_sql.get('x,y,z', name=['N', 'CA', 'C', 'O'], resSeq=list(intersect_res), chainID=[chain])),
            np.array(template_sql.get('x,y,z', name=['N', 'CA', 'C', 'O'], resSeq=list(intersect_res), chainID=[chain]))
        )

        # Superpose
        superpose_rmsd = pdb2sql.StructureSimilarity.get_rmsd(
            np.array(pdb2sql.pdb2sql(aligned_pdb).get('x,y,z', name=['N', 'CA', 'C', 'O'], resSeq=list(intersect_res), chainID=[chain])),
            np.array(template_sql.get('x,y,z', name=['N', 'CA', 'C', 'O'], resSeq=list(intersect_res), chainID=[chain]))
        )

        rmsd_dict[pdb_basename] = (superpose_rmsd, pdb2sql_rmsd)
    except ValueError:
        # Jank with multiple of the same atoms?
        print("Can't calc RMSD. Amount of atoms don't match!", file=sys.stderr)
        print(unaligned_pdb, template, file=sys.stderr)
...
def main():
    ...
    torch.set_num_threads(arguments.n_cores)
    ...
    if arguments.subfolders:
        ...
        pdbs_list += glob.glob(arguments.input+'/*.pdb')
    ...
    num_batches = math.ceil(len(pdbs_list)/arguments.batch_size)
    for i in range(num_batches):
        batch_start = i * arguments.batch_size
        if arguments.verbosity > 0:
            print(f"\nProcessing batch {i+1}/{num_batches}")
        data_processor = PDBdataset(
            pdbs_list[batch_start:batch_start+arguments.batch_size],
            arguments.template,
            arguments.residues,
            arguments.chain,
            arguments.n_cores,
    ...
    if arguments.rmsd_test:
        with mp.Manager() as manager:
            rmsd_dict = manager.dict()
            with mp.Pool(arguments.n_cores) as pool:
                list(tqdm(
                    pool.imap(
                        func=rmsd_test_pool,
                        iterable=zip(pdbs_list, repeat(arguments.template), repeat(arguments.output), repeat(rmsd_dict), repeat(data_processor.chain)),
                        chunksize=max(2, round(len(pdbs_list) / (arguments.n_cores * 16)))
                    ),
                    total=len(pdbs_list),
                    desc="Comparing RMSDs"
                ))
            with open("rmsd.tsv", "w", encoding='utf-8') as rmsd_file:
                rmsd_file.write(f"Template\t{arguments.template}\n")
                for key, value in rmsd_dict.items():
                    rmsd_file.write(f"{key}\t{value[0]}\t{value[1]}\n")
